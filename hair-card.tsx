...
export function HairCard({ character, updateCharacter }: HairCardProps) {
  return (
    <AccordionItem value="hair">
      <AccordionTrigger className="text-lg font-semibold">Hair</AccordionTrigger>
      <AccordionContent>
        <Card>
          <CardContent className="pt-6 grid grid-cols-2 md:grid-cols-4 gap-6">
            {(Object.keys(characterOptions) as (keyof typeof characterOptions)[]).filter(key => 
                ['hairType', 'hairLength', 'hairColor', 'beard'].includes(key)
            ).map((key) => (
