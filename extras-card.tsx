...
export function ExtrasCard({ character, updateCharacter }: ExtrasCardProps) {
  return (
    <AccordionItem value="extras">
      <AccordionTrigger className="text-lg font-semibold">Extras</AccordionTrigger>
      <AccordionContent>
        <Card>
          <CardContent className="pt-6 grid grid-cols-2 md:grid-cols-4 gap-6">
            {(Object.keys(characterOptions) as (keyof typeof characterOptions)[]).filter(key => 
                ['ears', 'horns', 'teeth', 'tongue'].includes(key)
            ).map((key) => (
